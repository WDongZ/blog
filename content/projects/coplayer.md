...
$Q$ 值的计算由两部分组成。首先是**空间成本**，即智能体位置与所需食材之间的欧几里得距离；其次是**状态转换成本**，系统会扫描场景快照（WorldStateSnapshot）以寻找最近的交互设备（如砧板或锅具），并将设备距离与操作耗时（如切菜所需的固定时长）累加。若关键道具在当前快照中缺失，系统将施加极大的惩罚项，从而在数学上抑制该目标产生的似然度。
